...
ENV HTTP_PROXY="http://si-bridge-1.ir-thr-dc1.saba-e.com:9999"
ENV HTTPS_PROXY="http://si-bridge-1.ir-thr-dc1.saba-e.com:9999"
...
FROM ubuntu:${UBUNTUVER} AS build-ffplayout

# use bash since we need source
...
# # Copy libnpp
# COPY --from=build-ffmpeg /usr/local/cuda-12.2/targets/x86_64-linux/lib/libnppc.so /lib/x86_64-linux-gnu/libnppc.so.12
# COPY --from=build-ffmpeg /usr/local/cuda-12.2/targets/x86_64-linux/lib/libnppig.so /lib/x86_64-linux-gnu/libnppig.so.12
# COPY --from=build-ffmpeg /usr/local/cuda-12.2/targets/x86_64-linux/lib/libnppicc.so /lib/x86_64-linux-gnu/libnppicc.so.12
# COPY --from=build-ffmpeg /usr/local/cuda-12.2/targets/x86_64-linux/lib/libnppidei.so /lib/x86_64-linux-gnu/libnppidei.so.12
# COPY --from=build-ffmpeg /usr/local/cuda-12.2/targets/x86_64-linux/lib/libnppif.so /lib/x86_64-linux-gnu/libnppif.so.12

# # Copy ffmpeg
# COPY --from=build-ffmpeg /app/workspace/bin/ffmpeg /usr/bin/ffmpeg
# COPY --from=build-ffmpeg /app/workspace/bin/ffprobe /usr/bin/ffprobe
# COPY --from=build-ffmpeg /app/workspace/bin/ffplay /usr/bin/ffplay
...
# Copy ffplayout
COPY --from=build-ffplayout /app/target/release/ffplayout /usr/bin/ffplayout
# Create run.sh
ENV DB=/db
COPY <<-EOT /run.sh
#!/bin/sh
...
if [ ! -f /db/ffplayout.db ]; then
    ffplayout -i -u admin -p admin --storage "/tv-media" --playlists "/playlists" --public "/public" --logs "/logging" --smtp-server "mail.example.org" --smtp-user "a" --smtp-port 10 --smtp-starttls --smtp-password "2" --mail "admin@example.org"
fi

/usr/bin/ffplayout -l "0.0.0.0:8787"
EOT

# Copy ffplayout service files & make directories
RUN mkdir -p /etc/ffplayout && \
    mkdir -p ${DB} && \
    mkdir -p "/tv-media" && \
    mkdir -p "/playlists" && \
    mkdir -p "/public" && \
    ...
    mkdir -p "/assets" && \
    chmod +x /run.sh
COPY --from=build-ffplayout /app/assets/ffplayout.service /etc/systemd/system/ffplayout.service
COPY --from=build-ffplayout /app/assets/dummy.vtt /assets/
COPY --from=build-ffplayout /app/assets/logo.png /assets/
COPY --from=build-ffplayout /app/assets/DejaVuSans.ttf /assets/
COPY --from=build-ffplayout /app/assets/FONT_LICENSE.txt /assets/
